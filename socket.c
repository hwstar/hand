...
int socket_create_listen(char *bindaddr, char *service, int family, int socktype, int (*addsock)(int sock, void *addr, int family, int socktype)) {
	...
	if((res = getaddrinfo(bindaddr, service, &hints, &list)) == -1){
		debug(DEBUG_EXPECTED, "socket_create_listen(): getaddrinfo failed: %s", gai_strerror(res));
		return FAIL;
	}
	...
	for(p = list; p != NULL; p = p->ai_next){ // Traverse the list
		...
		if((sock = socket(p->ai_family, p->ai_socktype, p->ai_protocol)) == -1){
			debug(DEBUG_EXPECTED,"socket_create_listen(): Call to socket failed with %s, continuing...",strerror(errno));
			continue;
		}		
		...
		if(p->ai_family == PF_INET6){
			...
			debug(DEBUG_EXPECTED,"socket_create_listen(): Setting IPV6_V6ONLY socket option");
		}
		...
		if(bind(sock, p->ai_addr, p->ai_addrlen) == -1){
			debug(DEBUG_EXPECTED,"socket_create_listen(): Bind failed with %s, continuing...", strerror(errno));
			close(sock);
			continue;
		}

		if(listen(sock, SOMAXCONN) == -1){
			debug(DEBUG_EXPECTED, "socket_create_listen(): Listen failed with %s, continuing...", strerror(errno));
			close(sock);
			continue;
			}
	...
	if(!sockcount){
		debug(DEBUG_EXPECTED, "socket_create_listen(): could not create, bind or listen on a socket");
		return FAIL;
	}
...
int socket_connect(char *socket_name) {
	...
	daemon_socket=socket (AF_UNIX, SOCK_STREAM, 0);
	if(daemon_socket == -1) {
		debug(DEBUG_UNEXPECTED, "Could not create socket: %s", strerror(errno));
		return FAIL;
	}
	...
	if(connect(daemon_socket, (struct sockaddr *) &sock_addr, sizeof(sock_addr)) != 0) {
		debug(DEBUG_UNEXPECTED, "Could not connect to socket '%s'.", socket_name);
		return FAIL;
	}
...
int socket_connect_ip(char *host, char *service, int family, int socktype) {
	...
	if((res = getaddrinfo(host, service, &hints, &list)) == -1){
		debug(DEBUG_UNEXPECTED, "socket_connect_ip(): getaddrinfo failed: %s", gai_strerror(res));
		return FAIL;
	}
	...
	if(!ipv4 && !ipv6){
		debug(DEBUG_UNEXPECTED,"socket_connect_ip(): Could not find a suitable IP address to connect to");
		return FAIL;
	}
	...
	if(sock == -1) {
		freeaddrinfo(list);
		debug(DEBUG_UNEXPECTED, "socket_connect_ip(): Could not create ip socket: %s", strerror(errno));
		return FAIL;
	}
	...
	if(connect(sock, (struct sockaddr *) p->ai_addr, p->ai_addrlen)) {
		freeaddrinfo(list);
		debug(DEBUG_UNEXPECTED, "socket_connect_ip(): Could not connect to inet host:port '%s:%s'.", host, service);
		return FAIL;
	}
...
int socket_write(int socket, void *buffer, int size, int timeout) {
	...
	for(sent=0; sent < size;) {
		...
		if(!socket_wait_write(socket, timeout)) {
			debug(DEBUG_EXPECTED, "Socket write timed out.");
			return FALSE;
		}
		...
		if(writeval <= 0) {
			debug(DEBUG_EXPECTED, "Write in socket write failed.");
			return FALSE;
		}
