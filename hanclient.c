...
void hanclient_connect_setup(char *pidpath, char *sockfilepath, char *service, char *host){
	...
	if(sockfilepath[0]){
		/* Make sure we are running hand (.pid file check). */
			...
			fatal("Cannot establish a unix domain connection, hand is not running, pid path = '%s'.", pidpath);
                sockFilePath = sockfilepath;
        }
	else if(service[0]){
		/* Make sure we have a host defined */
		if(host[0] == 0)
			fatal("Cannot establish an inet connection, host not defined in config file.");
		inetService = service;
		inetHost = host;	
	}
	else
		fatal("Can't determine a valid connect method (unix or ip) in the config file to reach the han server.");
}
...
int hanclient_send_command_return_res(Client_Command *client_command)
{
	...
	if(sock == -1)
		fatal_with_reason(errno, "Could not open socket to host: %s", inetHost);

	/* Set the socket file mode to non-blocking */

	if(fcntl(sock, F_SETFL, O_NONBLOCK) == -1)
		fatal_with_reason(errno, "Could not set socket to nonblocking");
	...
	if(i == 0)
		fatal("Socket time out error, writing client command");
  

	debug(DEBUG_EXPECTED,"packet sent, waiting for response");
	...
	if(i == 0)
		fatal("Socket time out error, waiting for response");

	/* Close the socket */

	socket_close(sock);

	return client_command->commstatus;
}
